...
log() {
    ...
    echo "$timestamp [$type] $msg" >> "$log_path"
}
...
calculate_size() {
    local folder="$1"
    local extension="$2"
    find "$folder" -type f -name "*${extension}" -exec ls -l {} + | awk '{total += $5} END {print total}'
}
...
cleaned_size=$(find "$source_path" -type f -name "*${log_extension}" -mtime +"$log_history" -exec du -k {} + | awk '{total += $1} END {print total}')
